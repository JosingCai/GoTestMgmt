	...
	tabs := components.Tabs().SetData([]map[string]template.HTML{
		{
			...
			"content": template.HTML(`<b>示例:</b>
                <p>1. 新建测试环境TEST</p>
                <p>2. 在 文件-API文件 菜单下：导入API文件(e.g.: TEST.json) </p>
                <p>3. 在 文件-用例文件 菜单下：导入测试用例文件（e.g.: TEST_V1.0.0_测试用例.xmind or TEST.xmind）</p>
                <p>4. 选中环境，点击Swagger导入API用例，在用例菜单下即可看到对应API数据</p>
                <p>5. 选中环境，点击Xmind导入测试用例，在用例菜单下即可看到对应测试用例数据</p>
                <p>6. 选中环境, 点击接口统计，在报表菜单下即可看到接口统计数据</p>
                <p>7. 选中环境, 点击用例统计，在报表菜单下即可看到用例统计数据</p>
                <p>8. 选中环境, 点击接口总览，在报表菜单下即可看到总个项目的接口统考数据</p>
                <p>注意: </p>
                <p> * 导入的文件名称与环境菜单下录入的环境信息项目名称保持一致</p>
                <p> * 手动添加用例时，关联项目录入的信息与环境信息中的项目名称保持一致</p>
                `),
		}, {
			"title": "接口用例说明",
